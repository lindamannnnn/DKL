...
- `getline(cin, s)`：s 是 **string**（第 07 课），不用写格子数
...
- 用 `cin >> s`，空格后的内容丢失
- 计数器忘记初始化为 0
- 循环写成 `i < 1005`：没读到的格子都是 `'\0'`，不会数错但白白多转上千圈；看到 `'\0'` 就停才聪明
...
while 条件里 `(pos = s.find(t, pos))` 是"先存再比"的缩写：先把查找结果存进 `pos`，再和 `string::npos` 比较，找到就继续、找不到就停（第 07 课"括号里的小动作"讲过）。
<!-- end-card -->
...
- `pos += n.size()` 写成 `pos++`：本例新串 "aa" 里还藏着 "a"，会反复匹配、死循环停不下来！
